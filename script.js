...
(function (ns) {
  ...
  ns.error = function (input, errorMsg) {
    input.before(
      `<div class="input-alert" style="color: ${ERROR_COLOR}">${errorMsg}</div>`
    );
  };
  ns.warning = function (input, errorMsg) {
    input.before(
      `<div class="input-alert" style="color: ${WARNING_COLOR}">${errorMsg}</div>`
    );
  };
...
(function (ns) {
            ...
            if (data && data["signup-email-validation"]) {
              ...
              if (validationResult.risk === "high") {
                if (
                  ...
                  ns.error(
                    self,
                    "Please do not use disposable email addresses."
                  );
                }
              ...
              if (validationResult.risk === "medium") {
                if (validationResult.reason === "mailbox_is_role_address") {
                  ns.warning(
                    self,
                    "Please use your personal work email address if possible."
                  );
                }
              }
            }
  ...
  function onSubmitUserSignupForm(response) {
    ...
    ns.post(
      ...
      formData,
      function () {
        window.location = SIGNUP_ACCOUNT_FORM_PAGE;
      },
      function (response) {
  ...
  function signupAccountFormHandler(form) {
    ...
    const API_VALIDATION_ENDPOINT = "/account/validation";
    ...
    $(
      COMPANY_DOMAIN_INPUT_SELECTOR + ", " + ACCOUNT_DOMAIN_INPUT_SELECTOR
